...
def prepare_data_splits(features, labels):
    ...
    train_loader = DataLoader(train_set, batch_size=128, shuffle=True)   # Create training loader with shuffling
    val_loader = DataLoader(val_set, batch_size=128, shuffle=False)      # Create validation loader w/out shuffling
    test_loader = DataLoader(test_set, batch_size=128, shuffle=False)    # Create test loader w/out shuffling
...
def execute_training(model, train_loader, val_loader, epochs=500):
    ...
    optimizer = optim.SGD(model.parameters(), lr=0.001)   # Create SGD optimizer with 0.001 learning rate
...
def create_visualizations(history):
   ...
   # Loss plot
    ...
    plt.legend()
    plt.grid(alpha=0.5)
    plt.savefig('results/loss_metrics_mini_batch.png')
    ...
    plt.legend()
    plt.grid(alpha=0.5)
    plt.savefig('results/accuracy_metrics_mini_batch.png')
    plt.close()  # Close the figure
